    ...
    for test_fold in range(0,TEST_SETS):
        for dev_fold in range(0,DEV_SETS):
            if test_fold != dev_fold:
                train_set, dev_set, test_set = get_data(dataset=DATASET,data_folds='data/'+DATASET+'/data_folds',test_fold=test_fold,dev_fold=dev_fold,image_folder="data/"+DATASET+'/'+image_folder,loss=LOSS,batch_size=BATCH_SIZE,num_outer_folds=10)
                if MODEL == "lr":
                    model = LinearRegression(NUM_BINS).to(processor)
                elif MODEL == "resnet":
                    model = ResNet18Regression().base_model.to(processor)
